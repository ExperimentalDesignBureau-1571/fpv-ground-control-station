...
Блок ТХ забезпечує можливість підключення JR-сумісного модуля передавача керування до концентратора виносного блоку для подальшого двостороннього обміну інформацією з пультом керування що підключений до блоку керування станцією. Загальний вигляд блоку ТХ з встановленим передавачем керування показано на картинці.

<img width="600" alt="Загальний вигляд блоку ТХ з встановленим передавачем керування" src="picture/general_view_of_the_TX_unit_with_the_transmitter_module_installed.png" />
...
|--------|------------|----------------|----------|
| XS1 | Підключення передавача керування | VCC, GND, CRSF (S.Port) | Підтримуються JR-сумісні передавачі керування |
| XS2 | Підключення до концентратора виносного блоку | +BAT, GND, диференційний сигнал стандарту RS-485 | |
| XS3 | Вихід живлення 8В | VCC, GND | Підключення потужних передавачів керування |
...
Двосторонній обмін інформацією між передавачем керування та пультом керування здійснюється за стандартом RS-485 через комутаційні лінії наземної станції. Сигнали керування через роз’єм XS2 надходять на конвертер інтерфейсів (модуль BARVINOK-5 RS-485 nano V2.1) який перетворює диференційний сигнал стандарту RS-485 в високошвидкісний сигнал протоколу CRSF та через пін S.Port роз’єму XS1 подає його на передавач керування. 
...
Зі сторони коротких пінів на плату адаптера накладено три шари самоклеючого електроізоляційного паперу для захисту від замикань контактних майданчиків плати адаптера на полігон загального проводу конвертора інтерфейсів.
...
| :--- | :--- | :---: | :---: |
| Гвинт | M3x8 DIN 965 | 2 штуки | Кріплення модуля BARVINOK-5 RS-485 nano V2.1 |
| Гайка | M3 DIN 934 | 2 штуки | Кріплення модуля BARVINOK-5 RS-485 nano V2.1 |
